    ...
    fn build_world() -> World {
        ...
        for x in -x_count..(x_count + 1) {
            let mut column = scene!(
                matrix: matrix4x4!(translation(x as f32, 0., 0.));
                bounding_volume: cube!(matrix: matrix4x4!(scale(scale, y_count as f32 + 0.5, scale)));
            );
            for y in -y_count..(y_count + 1) {
                ...
                column.add(sphere!(
                    matrix: matrix4x4!(translation(0., y as f32, 0.) scale_all(scale));
                    pattern: pattern;
                ));
            }
            root.add(column);
